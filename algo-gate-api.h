...
////    and define a function for thread to call to register
////    their targets. thread builds the gate, and array of structs
...
//
//    1. Define the algo, soj.h, previously in soj.c
//
...
//    That is determined by the need to add code in soj.c
...
//    algo-gate-api.c
//
//    12. Include algo-gate-api.h and soj.h inthe algo's source file.
//
...
// exclusion union and intersection operations are provided for convenience. In // some cases it may be desireable to
// use boolean algebra directly on the data to perform set operations. Sets can be represented as single elements, a
// bitwise OR of multiple elements, a bitwise OR of multiple set variables or constants, or combinations of the above.
// Examples:
//
// my_set = set_element;
...
static inline set_t set_union(set_t a, set_t b)
{
    return a | b;
}

// return set contained common elements from sets a & b
static inline set_t set_intsec(set_t a, set_t b)
{
    return a & b;
}

// all elements in set a are included in set b
static inline bool set_incl(set_t a, set_t b)
{
    return (a & b) == a;
}

// no elements in set a are included in set b
static inline bool set_excl(set_t a, set_t b)
{
    return (a & b) == 0;
}
...
typedef struct
{
    ...
    // Called once by each thread to allocate thread local buffers and
    // other initialization specific to threads.
    ...
    void (*build_block_header)(struct work *, uint32_t, uint32_t *, uint32_t *, uint32_t, uint32_t);
    ...
    // get the notification passed through
    void (*on_stratum_notify)(struct stratum_ctx *sctx, json_t *params);
...
int scanhash_generic(struct work *work, uint32_t max_nonce, uint64_t *hashes_done, struct thr_info *mythr);
...
int scanhash_4way_64in_32out(struct work *work, uint32_t max_nonce, uint64_t *hashes_done, struct thr_info *mythr);
...
int scanhash_8way_64in_32out(struct work *work, uint32_t max_nonce, uint64_t *hashes_done, struct thr_info *mythr);
